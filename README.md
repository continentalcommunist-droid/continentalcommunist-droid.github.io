...
The general XML sitemap is published at `/sitemap.xml`; the two-day Google News sitemap is published at `/news-sitemap.xml`. Both are advertised in `/robots.txt`. The Google-issued Search Console ownership file is published at `/googled448df15e8eebf72.html` and protected by the SEO validator; the `google_site_verification` configuration hook remains available as an optional meta-tag fallback.
