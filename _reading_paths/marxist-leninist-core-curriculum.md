---
layout: learning-path
title: Marxist-Leninist Core Curriculum
permalink: "/learn/pathways/marxist-leninist-core-curriculum/"
section_key: learn
section: learn
content_type: Learning Path
author: Continental Communist
thinkers:
- Karl Marx
- Friedrich Engels
- V. I. Lenin
- J. V. Stalin
region:
- Global
historical_period:
- Multiple periods
languages:
- English
description: A 24-week intensive study of the primary texts that establish the Marxist-Leninist framework of materialism, political economy, state power, organization, imperialism, national liberation, and socialist transition.
topics:
- Marxist Theory
- Historical Materialism
- Political Economy
- Imperialism
- Socialism
slug: marxist-leninist-core-curriculum
number: '09'
level: Advanced
effort: 24 weeks · 6–8 hours per week
prerequisite_summary: Complete Marxism Fundamentals or demonstrate equivalent preparation
prerequisites:
- Complete the Marxism Fundamentals pathway or be able to explain historical materialism, labor power, surplus value, class, and the state in your own words.
- Keep a study notebook that separates each author's claims, evidence, historical setting, and strategic conclusions.
- Read the assigned primary texts before consulting commentary or later doctrinal summaries.
introduction: >-
  This curriculum establishes a demanding primary-text core for the wider learning
  ecosystem. It begins with Marx and Engels on materialist method and capitalist
  production, then follows Lenin's development of state theory, revolutionary
  organization, concrete tactics, imperialism, national self-determination, and
  socialist transition. Stalin's later formulations appear at the end as historically
  influential systematizations to compare against the earlier texts, not as substitutes
  for reading them. The sequence is chronological where possible and repeatedly returns
  from doctrine to the concrete disputes and conditions in which each argument was made.
summary: >-
  By the end, you should be able to reconstruct the Marxist-Leninist framework as a
  connected argument: materialist method grounds the critique of political economy;
  class rule raises the problem of the state; revolutionary politics requires durable
  organization and concrete tactics; monopoly and finance capital reshape the world
  system; national and colonial questions alter revolutionary strategy; and socialist
  transition presents new political and economic contradictions. You should also be
  able to distinguish Marx's and Engels's arguments, Lenin's developments, and later
  Soviet codifications rather than collapsing them into a single timeless doctrine.
outcomes:
- Reconstruct the movement from historical materialism to the critique of capitalist production without substituting slogans for arguments.
- Explain Lenin's theory of the state and proletarian political power in relation to Marx's analysis of the Paris Commune.
- Distinguish class consciousness, political organization, party form, strategy, tactics, and mass initiative.
- Explain Lenin's account of monopoly, finance capital, imperialist rivalry, and war while identifying the historical evidence on which it relied.
- Relate national self-determination and colonial liberation to proletarian internationalism without treating every nationalism as politically identical.
- Compare the source texts with later Marxist-Leninist systematizations and identify both continuity and simplification.
stages:
- number: I
  title: Method, practice, and history
  weeks: Weeks 1–4
  focus: Establish the materialist method through Marx's break with contemplative materialism, the social production of consciousness, and Engels's retrospective account of that development.
  objectives:
  - Explain practice as a category of knowledge and social transformation.
  - Relate productive forces, relations of production, institutions, and consciousness without mechanical determinism.
  - Distinguish Marx's early formulations from Engels's later synthesis.
- number: II
  title: Capitalist production and accumulation
  weeks: Weeks 5–10
  focus: Move from wages and value to labor power, surplus value, machinery, accumulation, and the historical formation of capitalist property relations.
  objectives:
  - Trace the circuit from commodity exchange to capitalist production.
  - Explain absolute and relative surplus value.
  - Connect accumulation and technological change to class formation.
- number: III
  title: Class struggle, state, and revolution
  weeks: Weeks 11–14
  focus: Study Marx's concrete political histories and Lenin's reconstruction of the state problem through the experience of 1848, Bonapartism, and the Paris Commune.
  objectives:
  - Analyze coalitions and political forms within a class society.
  - Explain why Lenin distinguishes smashing the bourgeois state apparatus from simply taking office within it.
  - Compare dictatorship, democracy, transition, and the withering away of the state.
- number: IV
  title: Party, organization, and tactics
  weeks: Weeks 15–18
  focus: Follow the development from independent working-class organization to Lenin's arguments about political consciousness, revolutionary organization, alliances, and tactical flexibility.
  objectives:
  - Distinguish spontaneous struggle from socialist political consciousness.
  - Explain the relation among party, class, leaders, and masses.
  - Evaluate tactics in relation to concrete conditions rather than fixed formulas.
- number: V
  title: Imperialism and the national-colonial question
  weeks: Weeks 19–21
  focus: Examine monopoly capitalism, finance capital, world division, imperialist war, national self-determination, and the strategic place of colonial liberation.
  objectives:
  - Reconstruct Lenin's economic definition of imperialism.
  - Separate national equality, the right to secede, and support for a particular nationalist program.
  - Connect internationalism to struggles in oppressed and colonized nations.
- number: VI
  title: Socialist transition and systematization
  weeks: Weeks 22–24
  focus: Study early Soviet problems of administration, accounting, class transition, and economic organization before assessing later codifications of Leninism and dialectical materialism.
  objectives:
  - Identify contradictions Lenin expected to persist after a revolutionary transfer of power.
  - Distinguish emergency measures, transitional policies, and general theoretical claims.
  - Compare Stalin's systematizations with the Marx, Engels, and Lenin texts already completed.
readings:
- id: theses-feuerbach
  author: Karl Marx
  title: Theses on Feuerbach
  text: theses-on-feuerbach
  stage: Stage I · Method, practice, and history
  schedule: Week 1
  scope: Read the complete eleven theses twice. On the second pass, write one sentence explaining the problem addressed by each thesis and give special attention to Theses II, III, VIII, and XI.
  guiding_questions:
  - How does Marx critique earlier materialism for viewing reality only as an "object of contemplation" rather than as sensuous human activity (praxis)?
  - What is the significance of Thesis XI ("philosophers have only interpreted the world; the point is to change it") for materialist epistemology?
  url: "https://www.marxists.org/archive/marx/works/1845/theses/"
- id: german-ideology
  author: Karl Marx and Friedrich Engels
  title: The German Ideology, Part I — Feuerbach
  stage: Stage I · Method, practice, and history
  schedule: Week 2
  scope: Read Part I, sections A and B. Track the movement from real individuals and material production to division of labor, property, the state, ideology, and world history.
  guiding_questions:
  - What are the "first premises" of the materialist conception of history set forth by Marx and Engels?
  - How do the division of labor, private property, and the state generate alienation and ideological mystification?
  url: "https://www.marxists.org/archive/marx/works/1845/german-ideology/"
- id: critique-political-economy-preface
  author: Karl Marx
  title: Preface to A Contribution to the Critique of Political Economy
  stage: Stage I · Method, practice, and history
  schedule: Week 3
  scope: Read the complete preface. Diagram the relationship Marx proposes among productive forces, relations of production, legal and political forms, and social consciousness; then list what the passage does not claim.
  guiding_questions:
  - How does Marx formulate the relationship between the economic base (productive forces and relations of production) and the legal, political, and ideological superstructure?
  - Under what material conditions does an epoch of social revolution begin, according to Marx's formula?
  url: "https://www.marxists.org/archive/marx/works/1859/critique-pol-economy/preface.htm"
- id: ludwig-feuerbach
  author: Friedrich Engels
  title: Ludwig Feuerbach and the End of Classical German Philosophy
  stage: Stage I · Method, practice, and history
  schedule: Week 4
  scope: Read the complete text, concentrating on Parts II–IV and Engels's account of the break with Hegel, philosophical materialism, dialectics, and historical materialism.
  guiding_questions:
  - How did Marx and Engels extract the "rational kernel" of Hegelian dialectics while discarding its idealist philosophical shell?
  - What does Engels mean by the transition from classical philosophy to the working-class movement as the heir to philosophical inquiry?
  url: "https://www.marxists.org/archive/marx/works/1886/ludwig-feuerbach/"
- id: wage-labour
  author: Karl Marx
  title: Wage Labour and Capital
  text: wage-labour-and-capital
  stage: Stage II · Capitalist production and accumulation
  schedule: Week 5
  scope: Read the complete text. Reconstruct the wage relation using the distinctions among labor power, wages, commodities, and capital.
  guiding_questions:
  - Why is the commodity sold by the worker on the labor market "labor power" rather than "labor"?
  - How does capital accumulate by continually reinvesting surplus value extracted from living labor?
  url: "https://www.marxists.org/archive/marx/works/1847/wage-labour/"
- id: value-price-profit
  author: Karl Marx
  title: Value, Price and Profit
  text: value-price-and-profit
  stage: Stage II · Capitalist production and accumulation
  schedule: Week 6
  scope: Read the complete text. Focus on Marx's argument against the claim that wage increases simply determine general price increases and on the political limits of trade-union struggle.
  guiding_questions:
  - How does Marx refute Citizen Weston's claim that wage increases simply determine general price increases?
  - What is the distinction between the value of labor power (cost of subsistence) and the value created by the expenditure of that labor power?
  url: "https://www.marxists.org/archive/marx/works/1865/value-price-profit/"
- id: capital-commodity-labor-power
  author: Karl Marx
  title: Capital, Volume I — Commodity, money, and labor power
  text: capital-volume-one
  stage: Stage II · Capitalist production and accumulation
  schedule: Week 7
  scope: Read Chapters 1, 3, 4, 6, and 7. Define value, money, the general formula for capital, labor power, the labor process, and valorization in the order Marx introduces them.
  guiding_questions:
  - What is the "fetishism of commodities," and how does private exchange conceal social relations between producers as material relations between things?
  - How does money transform into capital through the circuit M–C–M', and why must surplus value originate within the sphere of production?
  url: "https://www.marxists.org/archive/marx/works/1867-c1/"
- id: capital-working-day-machinery
  author: Karl Marx
  title: Capital, Volume I — The working day and machinery
  text: capital-volume-one
  stage: Stage II · Capitalist production and accumulation
  schedule: Weeks 8–9
  scope: Read Chapters 10 and 15. Keep separate notes on absolute surplus value, relative surplus value, struggle over working time, machinery, cooperation, deskilling, and resistance.
  guiding_questions:
  - Why is the length of the working day the result of a protracted struggle between collective capital and collective labor?
  - How does machinery revolutionize production to extract relative surplus value while intensifying labor and subordinating the worker to the technical apparatus?
  url: "https://www.marxists.org/archive/marx/works/1867-c1/"
- id: capital-accumulation
  author: Karl Marx
  title: Capital, Volume I — Accumulation and primitive accumulation
  text: capital-volume-one
  stage: Stage II · Capitalist production and accumulation
  schedule: Week 10
  scope: Read Chapters 24–25 and Chapters 26–28, 31, and 32. Explain how reproduction, concentration, the reserve army of labor, dispossession, and colonial violence enter Marx's account of capitalist development.
  guiding_questions:
  - What is the "general law of capitalist accumulation," and why does capital accumulation inevitably generate an industrial reserve army of labor?
  - What was the historical role of "primitive accumulation" (enclosures, colonial conquest, enslavement) in separating direct producers from their means of subsistence?
  url: "https://www.marxists.org/archive/marx/works/1867-c1/"
- id: class-struggles-france
  author: Karl Marx
  title: The Class Struggles in France, 1848 to 1850
  stage: Stage III · Class struggle, state, and revolution
  schedule: Week 11
  scope: Read Parts I–III and Engels's 1895 introduction. List the classes and fractions Marx identifies, their political representatives, and the shifts in alliance across the sequence.
  guiding_questions:
  - How does Marx trace the real material interests behind the shifting parties and class fractions of the 1848 French Revolution?
  - Why was the defeat of the June insurrection of the Paris proletariat a pivotal turning point for European class struggle?
  url: "https://www.marxists.org/archive/marx/works/1850/class-struggles-france/"
- id: eighteenth-brumaire
  author: Karl Marx
  title: The Eighteenth Brumaire of Louis Bonaparte
  stage: Stage III · Class struggle, state, and revolution
  schedule: Week 12
  scope: Read the complete text. Track how class fractions, parties, inherited institutions, peasant isolation, ideology, and executive power combine in Marx's explanation of Bonapartism.
  guiding_questions:
  - What is "Bonapartism," and how can the executive state apparatus achieve relative autonomy when parliamentary parties represent deadlocked class fractions?
  - How did the parcelized property of the French peasantry lead them to seek salvation in an imperial executive authority?
  url: "https://www.marxists.org/archive/marx/works/1852/18th-brumaire/"
- id: civil-war-france
  author: Karl Marx
  title: The Civil War in France
  text: the-civil-war-in-france
  stage: Stage III · Class struggle, state, and revolution
  schedule: Week 13
  scope: Read Marx's three addresses and Engels's 1891 introduction. Extract the institutional features attributed to the Commune and distinguish historical description from programmatic conclusion.
  guiding_questions:
  - Why does Marx declare that "the working class cannot simply lay hold of the ready-made state machinery, and wield it for its own purposes"?
  - What structural measures adopted by the Paris Commune (recallable delegates, average workers' wages, suppression of the standing army) characterized a proletarian democracy?
  url: "https://www.marxists.org/archive/marx/works/1871/civil-war-france/"
- id: state-revolution
  author: V. I. Lenin
  title: The State and Revolution
  stage: Stage III · Class struggle, state, and revolution
  schedule: Weeks 13–14
  scope: Read the complete six chapters and postscript. Build a source table showing how Lenin uses specific passages from Marx and Engels to argue about the state, revolution, democracy, transition, and administration.
  guiding_questions:
  - How does Lenin define the state as an organ of class rule and an instrument for the reconciliation or suppression of class antagonisms?
  - What distinguishes the revolutionary "smashing" of the bourgeois state apparatus from the eventual "withering away" of the proletarian transitional state?
  url: "https://www.marxists.org/archive/lenin/works/1917/staterev/"
- id: gotha-programme
  author: Karl Marx
  title: Critique of the Gotha Programme
  stage: Stage III · Class struggle, state, and revolution
  schedule: Week 14 companion
  scope: Read the complete text with attention to labor, distribution, the lower and higher phases of communist society, the political transition period, and Marx's criticism of programmatic ambiguity.
  guiding_questions:
  - Why does Marx reject Lassalle's notion of an "undiminished proceeds of labor" and show what economic deductions must be made for social reproduction?
  - What characterizes the political transition period between capitalist and communist society as the "revolutionary dictatorship of the proletariat"?
  url: "https://www.marxists.org/archive/marx/works/1875/gotha/"
- id: communist-league-address
  author: Karl Marx and Friedrich Engels
  title: Address of the Central Committee to the Communist League
  stage: Stage IV · Party, organization, and tactics
  schedule: Week 15
  scope: Read the complete March 1850 address. Identify its arguments for independent working-class organization, temporary alliances, permanent revolution, and an autonomous political program.
  guiding_questions:
  - Why did Marx and Engels warn against subordinating independent working-class organization to democratic petty-bourgeois reformists?
  - What is the strategic meaning of their call for the revolution to be made "permanent"?
  url: "https://www.marxists.org/archive/marx/works/1847/communist-league/1850-ad1.htm"
- id: what-is-to-be-done
  author: V. I. Lenin
  title: What Is to Be Done?
  stage: Stage IV · Party, organization, and tactics
  schedule: Weeks 15–16
  scope: Read the preface and Chapters II–IV in full, then the conclusion. Reconstruct Lenin's polemic against Economism in its historical setting before generalizing claims about consciousness or organization.
  guiding_questions:
  - What is Lenin's critique of "Economism" and the worshipping of the "spontaneous" trade-union consciousness of the working class?
  - Why is a disciplined party of professional revolutionaries necessary to bring socialist political consciousness to the mass movement?
  url: "https://www.marxists.org/archive/lenin/works/1901/witbd/"
- id: two-tactics
  author: V. I. Lenin
  title: Two Tactics of Social-Democracy in the Democratic Revolution
  stage: Stage IV · Party, organization, and tactics
  schedule: Week 17
  scope: Read Chapters 1–6 and 12–13. Focus on class alliances, bourgeois and proletarian leadership in a democratic revolution, provisional government, and the relationship between stages of struggle.
  guiding_questions:
  - What was the divergence between Bolshevik and Menshevik perspectives on proletarian leadership in the 1905 bourgeois-democratic revolution?
  - What did Lenin mean by the "revolutionary-democratic dictatorship of the proletariat and the peasantry"?
  url: "https://www.marxists.org/archive/lenin/works/1905/tactics/"
- id: april-theses
  author: V. I. Lenin
  title: The Tasks of the Proletariat in the Present Revolution — April Theses
  stage: Stage IV · Party, organization, and tactics
  schedule: Week 17 companion
  scope: Read the complete theses. Compare the concrete political judgments of April 1917 with the framework of Two Tactics and identify what changed in the situation and in Lenin's strategic conclusion.
  guiding_questions:
  - Why did Lenin conclude upon returning to Russia in April 1917 that the democratic revolution had passed into its socialist stage?
  - What was the practical significance of the slogan "All Power to the Soviets" in relation to the state of dual power?
  url: "https://www.marxists.org/archive/lenin/works/1917/apr/04.htm"
- id: left-wing-communism
  author: V. I. Lenin
  title: Left-Wing Communism — An Infantile Disorder
  stage: Stage IV · Party, organization, and tactics
  schedule: Week 18
  scope: Read the complete text, including the chapters on trade unions, parliament, compromise, and the relation of leaders, parties, classes, and masses. Record each tactical claim together with the condition attached to it.
  guiding_questions:
  - Why does Lenin argue against dogmatic refusal to work inside reactionary trade unions or participate in bourgeois parliaments?
  - How does Lenin define the dialectical relationship among leaders, party, class, and the masses?
  url: "https://www.marxists.org/archive/lenin/works/1920/lwc/"
- id: imperialism
  author: V. I. Lenin
  title: Imperialism, the Highest Stage of Capitalism
  stage: Stage V · Imperialism and the national-colonial question
  schedule: Weeks 19–20
  scope: Read both prefaces and all ten chapters. Summarize the empirical evidence for concentration, monopoly, banking, finance capital, capital export, world division, and inter-imperialist rivalry before evaluating the synthesis.
  guiding_questions:
  - How did the transition from free competition to monopoly capitalism and finance capital fundamentally transform world politics?
  - Why does Lenin define imperialism as the "highest" and final stage of capitalism, characterized by decay, parasitism, and uneven development?
  url: "https://www.marxists.org/archive/lenin/works/1916/imp-hsc/"
- id: socialism-war
  author: V. I. Lenin and G. Zinoviev
  title: Socialism and War
  stage: Stage V · Imperialism and the national-colonial question
  schedule: Week 20 companion
  scope: Read Chapters I–II and the conclusion. Distinguish the class analysis of a war from moral opposition to war in general and identify Lenin's criteria for different kinds of war.
  guiding_questions:
  - What criteria distinguish predatory imperialist wars from legitimate wars of national liberation or defense?
  - What was the socialist principle of "revolutionary defeatism" in the context of inter-imperialist conflict?
  url: "https://www.marxists.org/archive/lenin/works/1915/s-w/"
- id: right-self-determination
  author: V. I. Lenin
  title: The Right of Nations to Self-Determination
  stage: Stage V · Imperialism and the national-colonial question
  schedule: Week 21
  scope: Read Sections 1, 4, 8, and 10. Define the right to self-determination precisely and distinguish recognition of that right from endorsing every demand made by a national bourgeoisie.
  guiding_questions:
  - Why is the political right of oppressed nations to self-determination (including secession) essential for genuine proletarian internationalism?
  - How does defending the right to self-determination differ from supporting the nationalist demands of the local bourgeoisie?
  url: "https://www.marxists.org/archive/lenin/works/1914/self-det/"
- id: national-colonial-theses
  author: V. I. Lenin
  title: Draft Theses on National and Colonial Questions
  stage: Stage V · Imperialism and the national-colonial question
  schedule: Week 21 companion
  scope: Read the complete draft. Note Lenin's distinctions between oppressor and oppressed nations, formal equality and class abolition, proletarian internationalism and national prejudice, and alliance and political subordination.
  guiding_questions:
  - Why did the Communist International emphasize the strategic alliance between the Western industrial proletariat and anti-colonial liberation movements?
  - How should communists maintain their organizational and ideological independence when participating in anti-imperialist united fronts?
  url: "https://www.marxists.org/archive/lenin/works/1920/jun/05.htm"
- id: immediate-tasks-soviet-government
  author: V. I. Lenin
  title: The Immediate Tasks of the Soviet Government
  stage: Stage VI · Socialist transition and systematization
  schedule: Week 22
  scope: Read the complete text. Track the problems of accounting, labor discipline, administration, productivity, specialist knowledge, and popular control that Lenin identifies after the seizure of power.
  guiding_questions:
  - How did early Soviet policy balance the technical necessity of specialist administrators and accounting with the development of workers' control?
  - Why did Lenin stress labor discipline and productivity as the decisive factors for the victory of the new social order?
  url: "https://www.marxists.org/archive/lenin/works/1918/mar/x03.htm"
- id: economics-politics-dictatorship
  author: V. I. Lenin
  title: Economics and Politics in the Era of the Dictatorship of the Proletariat
  stage: Stage VI · Socialist transition and systematization
  schedule: Week 22 companion
  scope: Read the complete short text. List the economic forms and classes Lenin argues continue to coexist during transition and explain why political struggle does not end with a change of state power.
  guiding_questions:
  - What coexisting economic forms (capitalism, small commodity production, socialism) did Lenin identify during the transitional dictatorship of the proletariat?
  - Why does class struggle continue, rather than disappear, after the conquest of political power?
  url: "https://www.marxists.org/archive/lenin/works/1919/oct/30.htm"
- id: foundations-leninism
  author: J. V. Stalin
  title: The Foundations of Leninism
  stage: Stage VI · Socialist transition and systematization
  schedule: Week 23
  scope: Read all nine lectures. For each section, identify the Lenin text or revolutionary problem being summarized, then mark where the formulation sharpens, compresses, or changes the earlier argument.
  guiding_questions:
  - How does Stalin define Leninism as "Marxism of the era of imperialism and the proletarian revolution"?
  - What distinct contributions of Lenin on the party, peasant alliance, national question, and strategy does Stalin synthesize?
  url: "https://www.marxists.org/reference/archive/stalin/works/1924/foundations-leninism/"
- id: dialectical-historical-materialism
  author: J. V. Stalin
  title: Dialectical and Historical Materialism
  stage: Stage VI · Socialist transition and systematization
  schedule: Week 24
  scope: Read the complete text alongside your notes from Theses on Feuerbach, The German Ideology, the 1859 Preface, and Ludwig Feuerbach. Identify what is preserved, what is formalized into propositions, and what complexity is lost.
  guiding_questions:
  - How does this text formalize the principles of dialectical and historical materialism into distinct philosophical propositions?
  - What aspects of Marx's open-ended method are codified or simplified in this systematization?
  url: "https://www.marxists.org/reference/archive/stalin/works/1938/09.htm"
lectures:
- id: praxis-history
  title: From praxis to the materialist conception of history
  duration: 50-minute lecture guide
  summary: Reconstructs the development from Marx's critique of contemplative materialism to a historical account of production, social relations, consciousness, and transformative practice.
  outline:
  - Feuerbach, materialism, and the active side developed by idealism
  - Real individuals, production, division of labor, and social consciousness
  - Base and superstructure as a research problem rather than a mechanical diagram
  - Engels's retrospective synthesis and the problem of later codification
- id: value-exploitation-accumulation
  title: Value, exploitation, and accumulation
  duration: 65-minute lecture guide
  summary: Connects the introductory wage texts to Capital's account of commodity value, labor power, surplus value, machinery, accumulation, and dispossession.
  outline:
  - Commodity form, abstract labor, value, and money
  - Labor power and the production of surplus value
  - Working time, machinery, cooperation, and class struggle
  - Accumulation, concentration, the reserve army, and primitive accumulation
- id: class-state-commune
  title: Class struggle, Bonapartism, and the Commune
  duration: 60-minute lecture guide
  summary: Uses Marx's political histories to explain why class rule takes multiple political forms and how the Commune reshaped the Marxist problem of the state.
  outline:
  - Class fractions, representation, alliances, and political crisis
  - Bonapartism and the relative autonomy of executive power
  - The institutional form of the Paris Commune
  - From Marx's accounts to Lenin's theory of smashing the state machine
- id: party-consciousness
  title: Political consciousness and revolutionary organization
  duration: 60-minute lecture guide
  summary: Situates What Is to Be Done? within the struggle against Economism and develops a precise account of the relations among class, movement, organization, and political theory.
  outline:
  - Economic struggle and political struggle
  - Spontaneity, consciousness, and the role of socialist theory
  - Organization under autocratic repression
  - Party, class, leaders, and masses across Lenin's later writings
- id: strategy-tactics
  title: Strategy, tactics, alliances, and concrete analysis
  duration: 55-minute lecture guide
  summary: Compares 1850, 1905, April 1917, and 1920 to show how Marxist-Leninist tactics change with class alignment, institutional conditions, and the development of struggle.
  outline:
  - Independent organization and temporary alliance
  - Democratic and socialist tasks in the 1905 framework
  - The April Theses and a changed revolutionary situation
  - Compromise, parliament, unions, and opposition to formulaic leftism
- id: monopoly-war
  title: Monopoly capitalism, imperialism, and war
  duration: 65-minute lecture guide
  summary: Reconstructs Lenin's synthesis of concentration, banking, finance capital, capital export, colonial division, unequal development, rivalry, and war.
  outline:
  - From free competition to monopoly
  - Banks, industrial capital, finance capital, and oligarchy
  - Export of capital and division of the world
  - Uneven development, rivalry, war, and the critique of ultra-imperialism
- id: nations-colonies
  title: Nations, colonies, and proletarian internationalism
  duration: 55-minute lecture guide
  summary: Clarifies Lenin's distinctions among national equality, the right to secede, concrete support for liberation movements, class independence, and international unity.
  outline:
  - The political content of self-determination
  - Oppressor nations and oppressed nations
  - National movements, class leadership, and conditional alliance
  - Colonial revolution and the strategic horizon of the Communist International
- id: transition-systematization
  title: Transition, administration, and the systematization of Leninism
  duration: 65-minute lecture guide
  summary: Examines the economic and administrative problems identified in early Soviet texts before comparing them with Stalin's later doctrinal syntheses.
  outline:
  - Political power and the persistence of mixed economic forms
  - Accounting, control, discipline, specialists, and mass participation
  - Foundations of Leninism as a post-revolutionary synthesis
  - What is gained and lost when a method becomes a fixed system of propositions
questions:
- How does Marx's concept of practice change the problem of whether a theory is true?
- Where do Marx and Engels describe determination, constraint, tendency, and agency differently, and why do those distinctions matter?
- How does the purchase of labor power make exploitation compatible with formally equal commodity exchange?
- What do Marx's histories of France reveal that cannot be derived from an abstract two-class model alone?
- Which claims in The State and Revolution depend on Marx's account of the Commune, and which are Lenin's own theoretical or strategic developments?
- How does the organizational argument of What Is to Be Done? change when read in the concrete conditions of tsarist illegality and the polemic against Economism?
- What changes between Two Tactics and the April Theses, and what does that comparison teach about concrete analysis?
- Which parts of Imperialism are definitions, which are historical evidence, and which are political conclusions?
- How does Lenin distinguish the right of national self-determination from political support for every nationalist movement?
- Where do Stalin's syntheses accurately organize the earlier texts, and where do they replace open historical analysis with fixed formulas?
glossary:
- term: Praxis
  definition: Conscious, purposive human activity through which people reproduce and transform social relations and test their understanding of the world.
- term: Dialectical materialism
  definition: A later name for the materialist approach that analyzes processes through development, contradiction, relation, and change rather than treating things as isolated and fixed.
- term: Historical materialism
  definition: An approach to social history centered on material production, class relations, institutions, struggle, and the historically specific forms of consciousness that arise within them.
- term: Mode of production
  definition: The historically specific unity of productive forces and relations of production through which a society organizes the production and reproduction of life.
- term: Labor power
  definition: The worker's capacity to labor, sold as a commodity under capitalism and capable of producing more value than is required for its own reproduction.
- term: Dictatorship of the proletariat
  definition: In the assigned texts, the transitional political rule of the working class after the displacement of bourgeois class power, not a synonym for one particular constitutional design.
- term: Class consciousness
  definition: An understanding of class position, common interests, political antagonisms, and historical tasks that develops through struggle, organization, theory, and experience.
- term: Vanguard party
  definition: A disciplined revolutionary organization that seeks to unite advanced workers, preserve strategic continuity, generalize experience, and contend for political leadership within the broader class movement.
- term: Democratic centralism
  definition: A principle combining internal deliberation and elected collective decision with unity in carrying out decisions; its institutional meaning has varied across parties and historical periods.
- term: Imperialism
  definition: In Lenin's synthesis, the monopoly stage of capitalism characterized by concentrated production, finance capital, capital export, international monopoly associations, and territorial division among major powers.
- term: National self-determination
  definition: The political right of a nation to determine its state relationship, including separation, which Lenin distinguishes from endorsing every nationalist program or abandoning class independence.
- term: Socialist transition
  definition: The contested period between capitalist class rule and a developed communist society in which inherited economic forms, classes, administrative practices, and international pressures continue to shape political struggle.
---
